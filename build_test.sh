...
set_C="ccache clang -v --target=aarch64-linux-musl -O3 -march=armv8.2-a+lse+crypto+dotprod -mcpu=cortex-a77 -flto=thin -Wno-error -ffunction-sections -fdata-sections"
set_HOSTC="ccache clang -v -O3 -flto=thin -Wno-error -ffunction-sections -fdata-sections"
...
#local_version_date_str="-$(date +%Y%m%d)-${GIT_COMMIT_ID}-MagicTime"
...
ZIP_FILENAME=MagicTime_${TARGET_DEVICE}_${KSU_ZIP_STR}_$(date +'%Y%m%d_%H%M%S')_anykernel3_${GIT_COMMIT_ID}.zip

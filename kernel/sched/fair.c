...
static inline long adjust_numa_imbalance(int imbalance, int dst_running, int dst_weight)
{
 ...
 *
 * Per the above update_tg_cfs_util() update_tg_cfs_runnable() are trivial
		 ...
		 struct task_struct *p, int dst_cpu)
{
	...
	for_each_cpu(cpu, pd_cpus) {
		...
		eff_util = max(min, min(eff_util, max));
...
static void set_cpus_allowed_fair(struct task_struct *p, const struct cpumask *mask, u32 flags)
{
	set_cpus_allowed_common(p, mask, flags);
